...
public class ProjectSaveService
{
    ...
    /// that contain a <c>&lt;id&gt;.bin</c> save file), sorted case-insensitively.
    /// </summary>
    ...
    public List<string> GetProjects()
    {
        ...
        return Directory.GetDirectories(ConfigService.ProjectsDir)
            .Where(dir => File.Exists(Path.Combine(dir, Path.GetFileName(dir) + ".bin")))
    ...
    /// <summary>
    /// Store project to <c>~/.rapidnovel/projects/&lt;id&gt;/&lt;id&gt;.bin</c>.
    ...
    public void StoreProject(string id, Project project)
    {
        ...
        var bytes = MessagePackSerializer.Serialize(project);
        File.WriteAllBytes(tempPath, bytes);
        File.Move(tempPath, path, overwrite: true);
    }

    /// <summary>
    /// Load project from <c>~/.rapidnovel/projects/&lt;id&gt;/&lt;id&gt;.bin</c>.
    /// </summary>
    ...
    public Project? LoadProject(string id)
    {
        ...
        return MessagePackSerializer.Deserialize<Project>(bytes);
    }

    /// <summary>
    /// Resolves a project <paramref name="id"/> to its save file path:
    /// <c>~/.rapidnovel/projects/&lt;id&gt;/&lt;id&gt;.bin</c>.
    /// Uses only the file name (stripping any directory parts) and enforces the <c>.bin</c> extension.
    /// </summary>
    private static string GetProjectPath(string id)
    {
        ...
        return Path.Combine(ConfigService.ProjectsDir, safeName, safeName + ".bin");
    }
}
